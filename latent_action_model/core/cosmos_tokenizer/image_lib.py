...
class ImageTokenizer(torch.nn.Module):
    ...
    def encode(self, input_tensor: torch.Tensor) -> tuple[torch.Tensor]:
        ...
        """
        # 优先使用独立 encoder；若不可用且存在完整模型，则从完整模型输出中提取潜变量
        if self._enc_model is not None:
        ...
        if self._full_model is not None:
            full_output = self._full_model(input_tensor)
            # 兼容 namedtuple(NetworkEval) 或标准 tuple/list 输出
            # 期望顺序为 (reconstructions, posteriors, latent)
            if hasattr(full_output, "latent") and hasattr(full_output, "posteriors"):
                return (full_output.latent, full_output.posteriors)
            if isinstance(full_output, (tuple, list)):
                if len(full_output) >= 3:
                    _, posteriors, latent = full_output[:3]
                    return (latent, posteriors)
                if len(full_output) >= 2:
                    # 退化情形：已经是 (latent, posteriors)
                    return tuple(full_output[:2])
                if len(full_output) == 1 and isinstance(full_output[0], torch.Tensor):
                    return full_output[0]
            if isinstance(full_output, torch.Tensor):
                return full_output
            raise RuntimeError("ImageTokenizer.encode: 无法从完整模型输出中提取潜变量")

        raise RuntimeError("ImageTokenizer.encode: 未初始化编码模型与完整模型")
